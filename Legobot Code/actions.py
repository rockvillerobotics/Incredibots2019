...
def deliver_ambulance_and_blocks():
    if c.SAFE_HOSPITAL == c.NEAR_ZONE:
        ...
        m.lift_arm()
        s.left_forwards_until_white(should_should_stop=False)
        s.left_forwards_until_black(should_should_stop=False)
...
def get_firefighters():
    if c.SAFE_HOSPITAL == c.NEAR_ZONE:
        ...
        s.right_forwards_until_black(should_should_stop=False)
        s.right_forwards_until_white()


    else:
        g.backwards_gyro_through_line_left(should_should_stop=False)
        g.backwards_gyro_until_black_left(should_should_stop=False)
        g.backwards_gyro(50)
        s.turn_left_until_white(should_should_stop=False)
        s.turn_left_until_black(should_should_stop=False)
        s.turn_left_until_black_right(should_should_stop=False)
...
def get_ambulance_and_blocks():
    ...
    s.align_far()
    g.drive_gyro_until_white_right(should_should_stop=False)
    g.drive_gyro_until_black_right()
    s.align_close()
    g.drive_gyro_until_black_third(should_should_stop=False)
    g.drive_gyro(100)
    ...
    g.drive_gyro_until_white_left(should_should_stop=False)
    g.drive_gyro_through_line_left(should_should_stop=False)
    g.drive_gyro_through_line_third()
    s.turn_left_until_black()
    s.lfollow_left_until_black_right_pid(should_should_stop=False)
    g.drive_gyro_through_line_right()
...
def go_to_firefighters():
    g.drive_gyro_until_black_fourth(should_should_stop=False)
    g.drive_gyro_until_white_fourth(should_should_stop=False)
    ...
    if s.isFourthOnBlack():
        ...
        s.turn_left_until_white()
        g.drive_gyro_until_black_third()
        s.turn_right_until_white(should_should_stop=False)
        ...
        g.drive_gyro_until_black_third()
        s.turn_right_until_white(should_should_stop=False)
    s.turn_right_until_black(should_should_stop=False)
...
def deliver_firefighters():
    ...
    s.lfollow_left_until_third_senses_black_pid(should_should_stop=False)
